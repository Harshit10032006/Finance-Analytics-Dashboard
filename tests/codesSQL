--Just for checking data  SQL Codes 
...
SELECT A.months,A.SUMM as Budget,B.op as Expenses, A.SUMM - B.op AS DIFF FROM (SELECT months,SUM(amount) AS SUMM FROM Budgets GROUP BY months) A 
JOIN 
(SELECT DATENAME(MONTH,amount_date)AS m,SUM(amount) as op 
 FROM Transactions WHERE type ='Expense' GROUP BY DATENAME(MONTH,amount_date))B ON A.months=B.m
